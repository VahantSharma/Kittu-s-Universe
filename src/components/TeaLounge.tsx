...
export const TeaLounge = ({ onNavigate }: TeaLoungeProps) => {
  ...
  return (
    <div className="min-h-screen p-6">
      {/* Header */}
      <div className="fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-sm border-b border-border/20 flex justify-between items-center p-6">
        <Button
          variant="ghost"
          onClick={() => onNavigate("home")}
          className="text-romantic hover:bg-secondary/50 border-0"
        >
        ...
        <div className="text-center">
          ...
          <p className="font-elegant text-romantic opacity-80">
            with your bestie Gigi ✨
          </p>
        </div>

        <div className="flex items-center gap-2 text-romantic">
          <Coffee className="w-5 h-5 animate-gentle-float" />
          <span className="font-elegant">Cozy Corner</span>
        </div>
      </div>

      {/* Chat Container */}
      <div className="max-w-4xl mx-auto pt-24">
        <Card className="h-[600px] flex flex-col romantic-glow bg-card/60 backdrop-blur-sm border-0">
          {/* Messages Area */}
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {messages.map((message) => (
              ...
              >
                <div
                  className={`max-w-[80%] p-4 rounded-3xl animate-bloom ${
                    message.sender === "user"
                      ? "bg-accent/80 text-romantic ml-auto romantic-glow"
                      : "bg-secondary/80 text-romantic romantic-glow"
                  }`}
                >
                  <p className="font-elegant leading-relaxed whitespace-pre-wrap">
                    {message.text}
                  </p>
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-secondary/80 text-romantic p-4 rounded-3xl romantic-glow animate-bloom">
          ...
          <div className="p-6 border-t border-border/20">
            <div className="flex gap-4">
              <Input
                ...
                className="flex-1 bg-background/50 border-border/30 text-romantic placeholder:text-romantic/50 focus:ring-accent rounded-2xl"
                disabled={isTyping}
              />
              <Button
                onClick={sendMessage}
                disabled={!currentMessage.trim() || isTyping}
                className="bg-accent/80 hover:bg-accent text-romantic border-0 hover-bloom px-6 rounded-2xl"
              >
                <Send className="w-4 h-4" />
              </Button>
            </div>

            <p className="text-xs text-romantic/60 mt-2 text-center font-elegant">
